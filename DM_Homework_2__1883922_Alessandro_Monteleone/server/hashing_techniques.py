...
class Shingling():
    ...
    def __int__(self, hash_function, k: int):
        ...
        for i in range(self.b):
            ...
            for j in range(len(elements)):
                ...
                for k in range(len(buckets[bucket]) - 1):
                    near_elements.add((buckets[bucket][k], j))
...
class CompareWithLSH():
    ...
    def compute_nearest_documents(self, documents):
        ...
        start_time = int(dt.microsecond / 1000)
        signatures = []
        ...
        dt = datetime.datetime.now()
        self.execution_times.append(int(dt.microsecond / 1000) - start_time)
        return result
    ...
    def compute_nearest_documents(self, documents, threshold: float):
        ...
        for i in range(len(shinglings) - 1):
            for j in range(i + 1, len(shinglings)):
                jaccard_similarity = (len(shinglings[i].union(shinglings[j])) /
                                      len(shinglings[i].intersection(shinglings[j])))
...
class Report():
    ...
    def build_report(self,df):
        ...
        jaccard_result = jaccard_comparator.compute_nearest_documents(documents)
